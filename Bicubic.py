...
for i in range (len(a)):
    with mirrored_strategy.scope():
        ...
        model = tf.keras.models.Sequential([
                tf.keras.layers.Conv2D(4, (3, 3), activation='elu', padding = 'same', input_shape=(512, 512, 1), kernel_regularizer=tf.keras.regularizers.l2(a[i])),
...
with mirrored_strategy.scope():
    ...
    model = tf.keras.models.Sequential([
            tf.keras.layers.Conv2D(4, (3, 3), activation='elu', padding = 'same', input_shape=(512, 512, 1), kernel_regularizer=tf.keras.regularizers.l2(a)),
...
testdata = loadmat('bicubictest.mat')
xtest = np.array(testdata["xtest"])
ytest = np.array(testdata["ytest"])

xtest = xtest.reshape(1000, 512, 512, 1)
...
savemat('BicubicPrediction.mat', {'ytesthat': ytesthat})
